...
// Code moved to wolf logistics, where it really belongs. //#TODO remove this script function and add dependency to logistics?
if (!isNil "wolf_logistics_ace_fnc_addLogistikArsenalActions") exitWith {call wolf_logistics_ace_fnc_addLogistikArsenalActions;};
...
#define SPAWNBOX_WITHLOAD(text, classname) \
    [ \
        [QGVAR(__COUNTER__), text + format[" (%1)", getNumber (configFile >> "CfgVehicles" >> classname >> "maximumLoad")], "", {[classname, true] call wolf_arsenalpoint_fnc_spawnEquipBox;}, {true}, {}] call ace_interact_menu_fnc_createAction, [], _target \
    ]
...
[
    ...
    [
        [QGVAR(__COUNTER__), "Spawn ACE...", "", {}, {true}, {
            params ["_target"];
           [
                SPAWNBOX_WITHLOAD("Spare Wheel", 'ACE_Wheel'),
                SPAWNBOX_WITHLOAD("Spare Track", 'ACE_Track'),
                SPAWNBOX_WITHLOAD("Jerrycan 20L", 'Land_CanisterFuel_F'),
                SPAWNBOX_WITHLOAD("Jerrycan 20L RHS", 'rhsusf_props_ScepterMFC_OD'),
                SPAWNBOX_WITHLOAD("Flextank 300L Forest", 'FlexibleTank_01_forest_F'),
                SPAWNBOX_WITHLOAD("Flextank 300L Sand", 'FlexibleTank_01_sand_F')
           ]
        }] call ace_interact_menu_fnc_createAction, [], _target
    ]
]